define({ "api": [
  ...
  },
  {
    ...
    "name": "Login",
    "group": "Auth",
    "description": "<p>The login method is used to retrieve a session token by sending the user credentials. The session token is then sent as a header in subsequent requests that require authentication. The session token lasts for the amount of time defined in the configuration file, and the duration is refreshed on each authenticated request.</p> <p>Whenever you authenticate by sending the token, the server responds with the &quot;Session-Valid-Until&quot; header, which contains a timestamp of the time when the session will expire.</p> <p>Your application should be ready to handle a 401 Unauthorized request, result of the session token expiring, at any time. If that happens, call this method again.</p> <p>A session can be manually terminated by calling the {post} /auth/logout method.</p>",
    "parameter": {
      "fields": {
        "Parameter": [
          {
          ...
          },
          {
            ...
            "field": "extended",
            "defaultValue": "false",
            "description": "<p>Whether or not the session is meant to be extended. Extended sessions last longer than regular sessions (their duration is also defined in the config file), and are intended to be explicitly requested by the user (for example, by checking a &quot;remember me&quot; checkbox).</p>"
          }
        ]
      }
    },
